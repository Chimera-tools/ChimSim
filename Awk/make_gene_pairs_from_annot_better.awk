...
# takes as input an annotation file in gtf or gff format with gene rows and their gene id in field no 10
# and outputs 4 gene pairs files (for randomization purposes for chimeras):
# - a file with non overlapping same chr, same str and ok gx order gene pairs (in 5' to 3' order as given) with their coord, their mid point and their distance
# - a file with non overlapping same chr, same str and ko gx order gene pair (in 5' to 3' order as given) with their coord, their mid point and their distance
# - a file with non overlapping same chr, diff str gene pairs (in 5' to 3' order as given) with their coord, their mid point and their distance
# - a file with the gene pairs that lie on different chromosomes (which is a huge file that takes a lot of time to be generated)
